...
def get_documents_folder():
    if platform.system() == "Windows":
        print(get_get_current_time(),"--     Running on Windows")
        return os.path.join(os.environ['USERPROFILE'], 'Documents')
    elif platform.system() == "Linux":
        print(get_get_current_time(),"--     Running on Linux")
        return os.path.join(os.path.expanduser("~"), "Documents")
    elif platform.system() == "Darwin":
        print(get_get_current_time(),"--     Running on macOS")
        return os.path.join(os.path.expanduser("~"), "DBID_Automation")
    else:
        print(f"{get_get_current_time()} --     Running on an unknown OS: {platform.system()}")
...
DBID_Automation = os.path.join(location, 'Report')
dir_path = DBID_Automation
...
def newest_file_in_dir(dir_path):
    ...
    for f in os.listdir(dir_path):
        ...
        if os.path.isfile and extension.lower() == '.xlsx':
...
print(get_get_current_time(),"--     New File : "+new_file)
...
# --- Oracle Connection ---
try:
    # oracledb.init_oracle_client(lib_dir="/Users/AF35861/Downloads/instantclient_23_3/network/admin")
    # username = "wdbs"
    ...
    ora_conn = oracledb.connect(user=username, password=password, dsn=dsn, config_dir=tns_admin_dir)
    print(get_get_current_time(),"--     Successfully connected to Oracle Database!")

    ora_cursor = ora_conn.cursor() 
    print(f"{get_get_current_time()},--     Oracle Database Version: {ora_conn.version}")
except Exception as err:
    print(f"{get_get_current_time()},--     Error connecting to database: {err}")
...
try:
    ...
    connection_url = "jdbc:jtds:sqlserver://sql-snow-dev.us.ad.wellpoint.com:10002/ServiceNow;domain=us;ssl=required;useLOBs=false;socketTimeout=20000;usentlmv2=true;"
    ...
    print(get_get_current_time(),"--     Successfully connected to SNOW Replica Database!")
...
for i in loop:
    if i==dbid:
        ...
        # ora_cursor.close()
        #connection.close()
        print(get_get_current_time(),'--     DBID Successfull')
    elif i==job:
        ...
        workbook.save(new_file)
        #connection.close()
        print(get_get_current_time(),'--     Job Completion Successfull')
        ...
        try:
            ...
            # ora_cursor.close()
            #connection.close()
            print(get_get_current_time(),'--     Error Message Successfull')
        ...
        cursor_sql = sql_conn.cursor()
        ticket_data = cursor_sql.execute(tickets)
        rows = cursor_sql.fetchall()
        # Load the Excel workbook
        workbook = openpyxl.load_workbook(new_file)
        # Get the target sheet
        ...
        ora_conn.close()
        sql_conn.close()
        print(get_get_current_time(),'--    Incident Tickets Query Successfull')
