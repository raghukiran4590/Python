...
#prod_ind_bat='Y'
prod_ind_bat='N'  #Manually set for testing purpose change it to sys.argv[1] while scheduling
...
try:
    ...
    conn_oracle = oracledb.connect(user=username, password=password, dsn=dsn, config_dir=tns_admin_dir)
    print("Successfully connected to Oracle Database!")

    cursor = conn_oracle.cursor()
    print("Line 121")
    # sys.exit("Exiting due to specific condition.")
    ...
    if len(tns_admin_dir) > 0:  
        oracledb.init_oracle_client(lib_dir=tns_admin_dir)
        print("Oracle Client initialized successfully.")
    conn_oracle_1 = oracledb.connect(user=username, password=password, dsn=dsn, config_dir=tns_admin_dir)
    ...
    body = {
        'staticData': {
            ...
            'subject': f"DocumentDB {prod_ind_bat} Load SUCCESSFULL",
